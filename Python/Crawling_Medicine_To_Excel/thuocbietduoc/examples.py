...
class ThuocBietDuoc:
  ...
  def create_excel_file(sheet_name, excel_name):
  ...
  def save_text_to_excel(sheet, text_data, row, column):
  ...
  def save_detail_drug_to_excel(sheet, line, detail_url):
    ...
    # sheet_name = default_url.split("/")[2][4:-7] # thuocbietduoc
    # excel_name_file = "{}.xls".format(sheet_name) # thuocbietduoc.xls
    ...
    with requests.Session() as s:
      ...
      ThuocBietDuoc.save_text_to_excel(sheet, detail_url, line, 0)
      ...
      ThuocBietDuoc.save_text_to_excel(sheet, ten_thuoc, line, 1)
      ...
      if not img:
        ...
        img = img.get("src")
      ThuocBietDuoc.save_text_to_excel(sheet, img, line, 2)
      ...
      ThuocBietDuoc.save_text_to_excel(sheet, nhom_thuoc, line, 3)
      ...
      ThuocBietDuoc.save_text_to_excel(sheet, dang_bao_che, line, 4)
      ...
      ThuocBietDuoc.save_text_to_excel(sheet, dong_goi, line, 5)
      ...
      ThuocBietDuoc.save_text_to_excel(sheet, thanh_phan, line, 6)
      ...
      ThuocBietDuoc.save_text_to_excel(sheet, sdk, line, 7)
      ...
      ThuocBietDuoc.save_text_to_excel(sheet, nha_san_xuat, line, 8)
      ...
      # print(">>>>>>>>>", nha_dang_ky)
      ThuocBietDuoc.save_text_to_excel(sheet, nha_dang_ky, line, 9)
      ...
      ThuocBietDuoc.save_text_to_excel(sheet, nha_phan_phoi, line, 10)
      ...
      ThuocBietDuoc.save_text_to_excel(sheet, chi_dinh, line, 11)
      ...
      ThuocBietDuoc.save_text_to_excel(sheet, lieuluong_cachdung, line, 12)
      ...
      ThuocBietDuoc.save_text_to_excel(sheet, chong_chi_dinh, line, 13)
      ...
      ThuocBietDuoc.save_text_to_excel(sheet, chu_y_de_phong, line, 14)
      ...
      ThuocBietDuoc.save_text_to_excel(sheet, duoc_luc, line, 15)
      ...
      ThuocBietDuoc.save_text_to_excel(sheet, duoc_dong_hoc, line, 16)
      ...
      ThuocBietDuoc.save_text_to_excel(sheet, tac_dung, line, 17)
      ...
      ThuocBietDuoc.save_text_to_excel(sheet, tac_dung_phu, line, 18)
      line = line + 1

  def create_excel_file(sheet_name, excel_name_file):
    wb = xlwt.Workbook(encoding='utf-8', style_compression = 0)

    ws = wb.add_sheet(sheet_name)

    headers = ("url", "ten_thuoc", "img", "nhom_thuoc", "dang_bao_che", "dong_goi", "thanh_phan", "sdk", "nha_san_xuat", "nha_dang_ky", "nha_phan_phoi", "chi_dinh", "lieuluong_cachdung", "chong_chi_dinh", "chu_y_de_phong", "duoc_luc", "duoc_dong_hoc", "tac_dung", "tac_dung_phu")
  ...
  def add_new_sheet(sheet_name, excel_name_file):
    try:
      open_wb = open_workbook(excel_name_file)

      sheet_names = open_wb.sheet_names()

      # make a copy of it
      wb = copy(open_wb)

      if sheet_name in sheet_names:
        sheet = sheet_names.remove(sheet_name)
        wb.save(excel_name_file)
      else:
        ...
        wb.save(excel_name_file)
        print("Wrong here????")
    # When file not found, i will CREATE new Excel File
    except FileNotFoundError:
      ThuocBietDuoc.create_excel_file(sheet_name, excel_name_file)
...
# sheet_drug_group_name = medicine_group_urls[0][1].split("/")[-2] # nhom-thuoc-1-0

# excel_name = default_url.split(".")[1]
  ...
  with requests.Session() as s:
    ...
    while True:
      ...
      if sheet_drug_group_name in sheet_names:
        sheet = sheet_names.remove(sheet_drug_group_name)
        wb.save(excel_name_file)
      else:
        # add sheet to workbook with existing sheets
        sheet = wb.add_sheet(sheet_drug_group_name)
