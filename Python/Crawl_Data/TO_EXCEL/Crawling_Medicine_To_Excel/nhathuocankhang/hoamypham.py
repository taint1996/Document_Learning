...
class CrawlingNhaThuocAnKhang:
  ...
  def add_new_sheet(self, sheet_name):
    ...
    workbook = open_workbook(self, formatting_info=True)
...
sheet_name = home[2].split("/")[-1] # hoa-my-pham
excel_name = home[2].split(".")[1] # nhathuocankhang

excel_name_file = "{}.xls".format(excel_name) # nhathuocankhang.xls

excel = CrawlingNhaThuocAnKhang.add_new_sheet(excel_name_file, sheet_name)

to_excel = CrawlingNhaThuocAnKhang.save_home_to_excel(url, excel_name_file, home[2], 1, 0)

# # # ######################################## hoa-my-pham ########################################
get_defaults_page = CrawlingNhaThuocAnKhang.start_request_get_list(home[2], url)
print("================= Default", get_defaults_page)
#### show more product list
urls_show_more_list_page = CrawlingNhaThuocAnKhang.start_requests_post_list(home[2], "https://www.nhathuocankhang.com/aj/Category/Products", url)

# # #### all url of hoamypham
...
for url in all_urls_of_tpcn_products:
  ...
  for data in soup.find_all('div', class_="rowinfo"):
    ...
    # TODO: review lại producer
    # get Producer - Nhà sản xuất
    # get_producer = data.find('div', class_='shortdesc').find('span')#.text
    # print("============Get Producer", get_producer)
    # print("============ producer span", data.find('div', class_='shortdesc').find('span'))
    ...
    #   get_producer = None
    # else:
      # get_producer = ("".join(get_producer.split(text_producer))) #TODO: need tab
    # producers.append(get_producer)

    # get Made in country - Sản xuất tại
    # TODO: need to fix th
    get_mades_in = data.find('div', class_='shortdesc')
    print("================== made in", get_mades_in)
    # text_made_in = "Sản xuất tại"
    # check_made_in = text_made_in in get_mades_in[-1].text

    # if len(get_mades_in) > 1 and check_made_in:
      # get_mades_in = ("".join(get_mades_in[-1].text.split(text_made_in))) # need tab
    # else:
    #   get_mades_in = None
  ...
  #   print(get_row_contents)
  #   # for tag in get_row_contents("style"):
  #   #   if tag is not None:
  #   #     tag.decompose()
  #   details_item_rowcontent.append(get_row_contents.text.strip())
...
images_to_excel = CrawlingNhaThuocAnKhang.save_to_excel(home[2], excel_name_file, images, 1, 3)
producers_to_excel = CrawlingNhaThuocAnKhang.save_to_excel(home[2], excel_name_file, producers, 1, 5)
mades_in_to_excel = CrawlingNhaThuocAnKhang.save_to_excel(home[2], excel_name_file, mades_in, 1, 6)
details_item_row_content_to_excel = CrawlingNhaThuocAnKhang.save_to_excel(home[2], excel_name_file, details_item_rowcontent, 1, 7)
